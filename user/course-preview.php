                    ...
                    <div class="tab-content content" id="course-tabContent">
                        <div class="tab-pane fade show active" id="course-intro" role="tabpanel" aria-labelledby="course-intro-tab">
                            <div class="d-flex align-items-center justify-content-between mb-4">
                                <div>
                                    <h4 style="font-size: 12px;"class=" mb-0  text-truncate-line-3 contentTitle"><?= ucwords($course_info['title']) ?></h4>
                                

                                </div>
                                <div class="dropdown">
                                    <button class="btn btn-secondary btn-xs dropdown-toggle" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
                                    Switch Courses
                                    </button>
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                    <?php 
                                        $sql = $db->query("SELECT * FROM course_subscribers WHERE user_id = '$uid' ORDER BY sn DESC LIMIT 15");
                                        while ($row = mysqli_fetch_array($sql)) {
                                            $courseInfo = getCourse($row['course_id']); ?>

                                            <li><a class="dropdown-item" href="course-preview.php?slug=<?= $courseInfo['slug'] ?> "><?= $courseInfo['title'] ?>  </a></li>
                                        <?php } ?>
                                    </ul>
                                    </div>
                            </div>
